...
const getUserById = async (req, res) => {
  ...
  try {
    ...
    if (profile) {
      return res
        .status(200)
        .json({ status: 200, message: "hello", data: profile });
    } else {
...
const getUserByPassword = async (req, res) => {
  ...
  try {
    ...
    const matchUser = await db
      .collection("users")
      .findOne({ firstName, password });

    console.log(matchUser);
    if (matchUser) {
      return res
        .status(200)
        .json({ status: 200, message: "logged in", data: matchUser });
    } else {
      return res.status(400).json({ status: 400, message: "user not found" });
    }
  } catch (err) {
...
const addNote = async (req, res) => {
  ...
  try {
    ...
    const result = await db
      .collection("sounds")
      .findOneAndUpdate({ _id: ObjectId(_id) }, { $set: { note } });
    if (result) {
      return res.status(200).json({ status: 200 });
    } else {
      return res.status(400).json({ status: 400, message: "note not added" });
    }
  } catch (err) {
    console.log(err);
  }
...
const fillForm = async (req, res) => {
  try {
    const { favSound, noise, _id } = req.body;
    console.log(req.body);
    const client = new MongoClient(MONGO_URI, options);
    await client.connect();
    const db = client.db("AudioPlacebo");
    ...
    const updateProfile = await db
      .collection("users")
      .updateOne({ _id }, { $set: { favSound: favSound, noise: noise } });
    const findUser = await db.collection("users").findOne({ _id });
...
const getPlacebos = async (req, res) => {
  try {
    ...
    await client.connect();
    const db = client.db("AudioPlacebo");
    const myPlacebos = await db.collection("sounds").find().toArray();
